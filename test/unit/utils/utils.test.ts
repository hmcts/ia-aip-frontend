...
describe('utils', () => {
  ...
  describe('asBooleanValue', () => {
    it('should be true', () => {
      expect(asBooleanValue(1)).to.equal(true);
      expect(asBooleanValue('1')).to.equal(true);
      expect(asBooleanValue('true')).to.equal(true);
      expect(asBooleanValue('True')).to.equal(true);
      expect(asBooleanValue('TRUE')).to.equal(true);
      expect(asBooleanValue('yes')).to.equal(true);
      expect(asBooleanValue('Yes')).to.equal(true);
      expect(asBooleanValue('YES')).to.equal(true);
      expect(asBooleanValue('on')).to.equal(true);
      expect(asBooleanValue('On')).to.equal(true);
      expect(asBooleanValue('ON')).to.equal(true);
    });
    it('should be false', () => {
      expect(asBooleanValue(0)).to.equal(false);
      expect(asBooleanValue('0')).to.equal(false);
      expect(asBooleanValue('false')).to.equal(false);
      expect(asBooleanValue('False')).to.equal(false);
      expect(asBooleanValue('FALSE')).to.equal(false);
      expect(asBooleanValue('no')).to.equal(false);
      expect(asBooleanValue('No')).to.equal(false);
      expect(asBooleanValue('NO')).to.equal(false);
      expect(asBooleanValue('off')).to.equal(false);
      expect(asBooleanValue('Off')).to.equal(false);
      expect(asBooleanValue('OFF')).to.equal(false);
    });
    it('should be false if its null or undefined', () => {
      expect(asBooleanValue(null)).to.equal(false);
      expect(asBooleanValue(undefined)).to.equal(false);
    });

    it('should be false for special cases', () => {
      expect(asBooleanValue('')).to.equal(false);
      expect(asBooleanValue('undefined')).to.equal(false);
      expect(asBooleanValue('null')).to.equal(false);
    });
  ...
  describe('nowAppealDate', () => {
    it('gets an appeal date for today', () => {
      ...
      expect(nowAppealDate().year).to.equal(new Date().getFullYear().toString(10));
      expect(nowAppealDate().month).to.equal(month.toString(10));
      expect(nowAppealDate().day).to.equal(new Date().getDate().toString(10));
    });
  });

  describe('toIsoDate', () => {
    it('converts date correctly', () => {
      expect(toIsoDate({ year: '2020', month: '1', day: '1' })).to.equal('2020-01-01');
      expect(toIsoDate({ year: '2020', month: '12', day: '30' })).to.equal('2020-12-30');
    });

    it('handles leap year', () => {
      expect(toIsoDate({ year: '2020', month: '2', day: '29' })).to.equal('2020-02-29');
    });

    it('handles summer time year', () => {
      expect(toIsoDate({ year: '2020', month: '3', day: '30' })).to.equal('2020-03-30');
    });
  });
  ...
  describe('hasPendingTimeExtension', () => {
    ...
    it('does not have inflight appeals if no time extensions', () => {
      ...
      expect(inflightTimeExtension).to.equal(false);
    });
    ...
    it('has inflight appeal', () => {
      ...
      expect(inflightTimeExtension).to.equal(true);
    });
  });
  ...
  describe('formatTextForCYA', () => {
    it('converts \n to <br/>', () => {
      ...
      expect(actualText).to.equal('A_string<br />\nwith_a_new_line');
    });

    it('converts spaces', () => {
      const actualText = formatTextForCYA('A string');
      expect(actualText).to.equal('A&nbsp;string');
    });

    it('converts indents', () => {
      const actualText = formatTextForCYA('A string\n  with an indent');
      expect(actualText).to.equal('A&nbsp;string<br />\n&nbsp;&nbsp;with&nbsp;an&nbsp;indent');
    });
  });
  ...
  describe('getFtpaApplicantType', () => {
    ...
    it('documentIdToDocStoreUrl should retrieve the doc store url using key', () => {
      ...
      expect(result).to.equal('http://someDocumentUrl/');
    });
    ...
    it('isRemissionDecisionDecided', () => {
      ...
      testData.forEach(({
        ...
        it(`should be ${description}`, () => {
          ...
          expect(isRemissionDecisionDecided(req as Request, true)).to.deep.equal(expectedResponse);
        });
      });
    });
