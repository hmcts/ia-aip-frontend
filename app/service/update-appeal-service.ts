...
export default class UpdateAppealService {
  ...
  mapCcdCaseToAppeal(ccdCase: CcdCaseDetails): Appeal {
    ...
    if (yesNoToBool(caseData.submissionOutOfTime)) {
      ...
      if (caseData.applicationOutOfTimeDocument && caseData.applicationOutOfTimeDocument.document_filename) {
        ...
        const documentMapperId: string = this._documentManagementService.addToDocumentMapper(caseData.applicationOutOfTimeDocument.document_url, caseData.applicationOutOfTimeDocument.document_filename, documentMap);
        outOfTimeAppeal = {
          ...outOfTimeAppeal,
          evidence: {
    ...
    if (caseData.reasonsForAppealDocuments) {
      ...
      caseData.reasonsForAppealDocuments.forEach(document => {
        const documentMapperId: string = this._documentManagementService.addToDocumentMapper(document.value.document.document_url, document.value.document.document_filename, documentMap);
    ...
    const appeal: Appeal = {
      ...
      stf24wPreviousStatusWasYesAutoGenerated: caseData.stf24wPreviousStatusWasYesAutoGenerated
    };
    return appeal;
  }
  ...
  convertToCcdCaseData(appeal: Appeal, paymentsFlag = false, refundFlag = false) {
    ...
    caseData = {
      ...
      ...appeal.hasNlrSubmitted && { hasNlrSubmitted: appeal.hasNlrSubmitted },
  ...
  private mapSupportingDocumentToEvidence(evidence: SupportingDocument, documentMap: DocumentMap[]) {
    const documentMapperId: string = this._documentManagementService.addToDocumentMapper(evidence.document_url, evidence.document_filename, documentMap);
    return {
      fileId: documentMapperId,
  ...
  private mapSupportingDocumentsToEvidence(evidence: Collection<SupportingDocument>[], documentMap: DocumentMap[]) {
    return evidence.map(doc => {
      const documentMapperId: string = this._documentManagementService.addToDocumentMapper(doc.value.document_url, doc.value.document_filename, documentMap);
      return {
        fileId: documentMapperId,
  ...
  private mapDocWithMetadataToEvidence = (doc: Collection<DocumentWithMetaData>, documentMap: DocumentMap[]): Evidence => {
    const fileId = this._documentManagementService.addToDocumentMapper(doc.value.document.document_url, doc.value.document.document_filename, documentMap);
    return {
      fileId,
