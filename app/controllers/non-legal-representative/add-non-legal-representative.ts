...
import { addNonLegalRepStatus, buildSectionObject } from '../../utils/tasks-utils';
...
function getNonLegalRepTaskStatus(status: ApplicationStatus) {
  const stepOne = buildSectionObject('stepOne', ['provideNlrEmail'], status);
  const stepTwoList = status.isNlrSameAsSponsor ?
    ['isNlrSameAsSponsor', 'provideNlrName', 'provideNlrAddress', 'provideNlrPhone']
    : ['provideNlrName', 'provideNlrAddress', 'provideNlrPhone'];
  const stepTwo = buildSectionObject('stepTwo', stepTwoList, status);
  const stepThree = buildSectionObject('stepThree', ['checkAndSend'], status);
  return [
    stepOne,
    stepTwo,
    stepThree
...
function getTaskList(req: Request, res: Response, next: NextFunction) {
  try {
    const status: ApplicationStatus = addNonLegalRepStatus(req.session.appeal);
    const statusOverview = getNonLegalRepTaskStatus(status);

    return res.render('non-legal-rep/task-list.njk', {
...
function postAddAnotherNonLegalRepresentative(updateAppealService: UpdateAppealService) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const appealUpdated: Appeal = await updateAppealService.submitEventRefactored(Events.PROVIDE_NON_LEGAL_REP_DETAILS,
        appeal, req.idam.userDetails.uid, req.cookies['__auth-token']);
      req.session.appeal = {
        ...appeal,
        ...appealUpdated
      };
      const redirectPage = getRedirectPage(editingMode, paths.nonLegalRep.provideNlrDetailsCheckAndSend, false, paths.nonLegalRep.addNonLegalRep);
...
function postNlrPhoneNumber(updateAppealService: UpdateAppealService) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
...
function setupNonLegalRepresentativeControllers(middleware: Middleware[], updateAppealService: UpdateAppealService,): Router {
  const router = Router();
  router.get(paths.nonLegalRep.addNonLegalRep, middleware, getTaskList);
  ...
  router.post(paths.nonLegalRep.provideNlrName, middleware, postNlrName(updateAppealService));
  router.get(paths.nonLegalRep.provideNlrAddress, middleware, getNlrAddress);
  router.post(paths.nonLegalRep.provideNlrAddress, middleware, postNlrAddress(updateAppealService));
  router.get(paths.nonLegalRep.provideNlrPhoneNumber, middleware, getNlrPhoneNumber);
  router.post(paths.nonLegalRep.provideNlrPhoneNumber, middleware, postNlrPhoneNumber(updateAppealService));
  router.get(paths.nonLegalRep.provideNlrIsSamePerson, middleware, getSamePerson);
  router.post(paths.nonLegalRep.provideNlrIsSamePerson, middleware, postSamePerson(updateAppealService));
  ...
  router.post(paths.nonLegalRep.addAnotherNonLegalRep, middleware, postAddAnotherNonLegalRepresentative(updateAppealService));
