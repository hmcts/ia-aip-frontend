...
const loggerInstance: LoggerInstance = LoggerJs.getLogger('jwt-utils.ts');
...
export function isJWTExpired(jwtToken: string) {
  ...
  if (!jwtToken) {
    loggerInstance.info('S2S token is null or empty.');
    return true;
  }
  ...
  if (decoded && decoded.exp < (currentTime + offset)) {
    loggerInstance.info('S2S token has expired.');
    isExpiredToken = true;
  }
