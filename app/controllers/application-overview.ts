...
import { hasPendingTimeExtension, isFtpaFeatureEnabled, yesNoToBool } from '../utils/utils';
...
function getApplicationOverview(updateAppealService: UpdateAppealService) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { appealReferenceNumber, appealStatus, paymentStatus, paAppealTypeAipPaymentOption, stf24wPreviousStatusWasYesAutoGenerated } = req.session.appeal;
      ...
      const stagesStatus = buildProgressBarStages(appealStatus, paymentStatus, is24WeeksTimeline);
