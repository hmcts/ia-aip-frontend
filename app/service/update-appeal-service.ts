...
export default class UpdateAppealService {
  ...
  mapCcdCaseToAppeal(ccdCase: CcdCaseDetails): Appeal {
    ...
    if (yesNoToBool(caseData.submissionOutOfTime)) {
      ...
      if (caseData.applicationOutOfTimeDocument && caseData.applicationOutOfTimeDocument.document_filename) {
        ...
        const documentMapperId: string = this._documentManagementService.addToDocumentMapper(caseData.applicationOutOfTimeDocument.document_url, caseData.applicationOutOfTimeDocument.document_filename, caseData.applicationOutOfTimeDocument.dateUploaded, documentMap);
        outOfTimeAppeal = {
          ...outOfTimeAppeal,
          evidence: {
    ...
    const appeal: Appeal = {
      ...
      documentMap: documentMap.map(doc => {
        return {
          ...
          name: doc.name,
          documentUploadDate: doc.documentUploadDate
        };
      }),
  ...
  private mapSupportingDocumentToEvidence(evidence: SupportingDocument, documentMap: DocumentMap[]) {
    const documentMapperId: string = this._documentManagementService.addToDocumentMapper(evidence.document_url, evidence.document_filename, evidence.dateUploaded, documentMap);
    return {
      fileId: documentMapperId,
  ...
  private mapSupportingDocumentsToEvidence(evidence: Collection<SupportingDocument>[], documentMap: DocumentMap[]) {
    return evidence.map(doc => {
      const documentMapperId: string = this._documentManagementService.addToDocumentMapper(doc.value.document_url, doc.value.document_filename, doc.value.dateUploaded, documentMap);
      return {
        fileId: documentMapperId,
