...
The application can be run against preview instances of CCD, Idam, etc built from an ia-case-api PR.

This only has to be run once per local instance - DO NOT commit the aat.env file generated by this command to source control, where `PR_NUMBER` is the pr-number of your ia-case-api PR.
```
PR_NUMBER=1234 yarn get-preview-dev-variables
```
Then to run the app pointed to preview (hosted at https://localhost:3000):
```
yarn dev:preview
```

Now the app will be running on https://localhost:3000 and pointed to the preview environment of ia-case-api. To be clear, it is now running with your local version of ia-aip-frontend but talking to the preview environment of ia-case-api so when submitting events or validating, it will be using CCD and other services in that preview environment.
