  ...
  switch (currentAppealStatus) {
    ...
    case 'appealSubmitted':
      if (dlrmFeeRemissionFlag && remissionDecisionEventIsTheLatest(req) && !isLateRemissionRequest) {
        ...
        doThisNextSection = getFeeRemissionParagraph(deadLineDate, is24WeeksTimeline);
      } else {
        doThisNextSection = {
          descriptionParagraphs: [
...
function getFeeRemissionParagraph(deadLineDate: string, is24WeeksTimeline: boolean) {
