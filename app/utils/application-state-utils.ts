...
async function getAppealApplicationNextStep(req: Request) {
  ...
  const is24WeeksTimeline = yesNoToBool(req.session.appeal.stf24wPreviousStatusWasYesAutoGenerated);
  ...
  switch (currentAppealStatus) {
    ...
    case 'appealSubmitted':
      if (dlrmFeeRemissionFlag && remissionDecisionEventIsTheLatest(req) && !isLateRemissionRequest) {
        ...
        doThisNextSection = getFeeRemissionParagraph(deadLineDate, is24WeeksTimeline, isNonLegalRep);
      } else {
        doThisNextSection = {
          descriptionParagraphs: [
...
function getRemissionDecisionParagraphs(req: Request, is24WeeksTimeline: boolean) {
  let doThisNextSection: DoThisNextSection;
...
function getFeeRemissionParagraph(deadLineDate: string, is24WeeksTimeline: boolean, isNonLegalRep: boolean) {
  const i18n = getI18n(isNonLegalRep);
  const doThisNextSection: DoThisNextSection = {
    descriptionParagraphs: [
      ...
      i18n.pages.overviewPage.doThisNext.appealSubmittedDlrmFeeRemission.feeDetails,
      i18n.pages.overviewPage.doThisNext.appealSubmittedDlrmFeeRemission.tribunalCheck,
        ...(is24WeeksTimeline ? [] : [i18n.pages.overviewPage.doThisNext.appealSubmittedDlrmFeeRemission.dueDate])
    ],
    cta: null,
    allowedAskForMoreTime: false
  };
