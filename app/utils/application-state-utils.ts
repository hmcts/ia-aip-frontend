...
async function getAppealApplicationNextStep(req: Request) {
  ...
  const is24Weeks = yesNoToBool(req.session.appeal.stf24wPreviousStatusWasYesAutoGenerated);
  ...
  switch (currentAppealStatus) {
    ...
    case 'appealSubmitted':
      if (dlrmFeeRemissionFlag && remissionDecisionEventIsTheLatest(req) && !isLateRemissionRequest) {
        doThisNextSection = getRemissionDecisionParagraphs(req, is24Weeks);
...
function getRemissionDecisionParagraphs(req: Request, is24Weeks: boolean) {
  let doThisNextSection: DoThisNextSection;
...
function getFeeRemissionParagraph(deadLineDate: string, is24Weeks: boolean) {
  const doThisNextSection: DoThisNextSection = {
    descriptionParagraphs: [
      i18n.pages.overviewPage.doThisNext.appealSubmittedDlrmFeeRemission.detailsSent,
      i18n.pages.overviewPage.doThisNext.appealSubmittedDlrmFeeRemission.feeDetails,
      i18n.pages.overviewPage.doThisNext.appealSubmittedDlrmFeeRemission.tribunalCheck,
        ...(is24Weeks ? [] : [i18n.pages.overviewPage.doThisNext.appealSubmittedDlrmFeeRemission.dueDate])
    ],
    cta: null,
    allowedAskForMoreTime: false
  };
